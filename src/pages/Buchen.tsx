...
const bookingSchema = z.object({
  ...
  tier: z.enum(["economy", "comfort"], { required_error: "Bitte wähle einen Reisetyp" }),
  notes: z.string().max(1000).optional(),
});

type BookingForm = z.infer<typeof bookingSchema>;

const PRICES = { economy: 990, comfort: 1490 } as const;
...
const Buchen = () => {
  ...
  const {
    ...
    defaultValues: { persons: 1, tier: "comfort", notes: "" },
  });
  ...
  const totalPrice = useMemo(() => {
    const p = persons || 1;
    const pricePerPerson = tier ? PRICES[tier] : PRICES.comfort;
  ...
  const onSubmit = async (data: BookingForm) => {
    ...
    try {
      const pricePerPerson = PRICES[data.tier];
      const total = data.persons * pricePerPerson;
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-card rounded-2xl border border-border p-6 md:p-8 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                ...
                onValueChange={(v) => setValue("tier", v as "economy" | "comfort", { shouldValidate: true })}
            ...
            <div className="rounded-xl bg-muted p-4 text-center">
              <p className="text-lg font-bold text-primary">
                Gesamtpreis: {totalPrice.toLocaleString("de-DE")} € ({persons || 1} {(persons || 1) === 1 ? "Person" : "Personen"} × {tier ? PRICES[tier].toLocaleString("de-DE") : "1.490"} €)
              </p>
            </div>
